 ...
 */
public class BitCrushPcmAudioFilter implements FloatPcmAudioFilter {
    private final FloatPcmAudioFilter downstream;
    private float volume = 1.0f;
    private int fac = 2;
    private int fac2 = 8;

    public BitCrushPcmAudioFilter(FloatPcmAudioFilter downstream) {
        this.downstream = downstream;
    }

    @Deprecated
    public BitCrushPcmAudioFilter(FloatPcmAudioFilter downstream, int channelCount, int bufferSize) {
        this(downstream);
    }

    @Deprecated
    public BitCrushPcmAudioFilter(FloatPcmAudioFilter downstream, int channelCount) {
        this(downstream);
    }
     ...
     */
    public BitCrushPcmAudioFilter setVolume(float volume) {
//        if(volume <= 0) {
...
//        }
     ...
     * @return {@code this}, for chaining calls
     */
    public BitCrushPcmAudioFilter updateVolume(FloatToFloatFunction function) {
...
//                array[i] = Math.round(array[i] * fac) / (float)fac;
            }
        }
    ...
    public static class Config extends AudioFilterConfig {
        ...
        public AudioFilter build(AudioDataFormat format, UniversalPcmAudioFilter output) {
            BitCrushPcmAudioFilter audioFilter = new BitCrushPcmAudioFilter(output);
            return audioFilter;
        }

        @Override
        public String getDescription() {
            return String.format("Bitcrush");
        }
    }
}
