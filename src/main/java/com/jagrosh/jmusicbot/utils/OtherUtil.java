...
public class OtherUtil {
     ...
     * any time the bot tries to access this path, it will instead start in the
    ...
    public static void checkVersion(Prompt prompt) {
        ...
        if (latestVersion != null && !latestVersion.equals(version)) {
            prompt.alert(Prompt.Level.WARNING, "JMusicBotバージョン",
        ...
        try {
            Response response = new OkHttpClient.Builder().build()
                    .newCall(new Request.Builder().get()
                            .url("https://api.github.com/repos/jagrosh/MusicBot/releases/latest").build())
                    .execute();
