...
public abstract class MusicCommand extends Command {
    ...
    protected void execute(CommandEvent event) {
        ...
        if(tChannel != null && !event.getTextChannel().equals(tChannel)) {
            ...
            event.replyInDm(
                event.getClient().getError() + " You can only use that command in " + tChannel.getAsMention() + "!");
            return;
        }
        bot.getPlayerManager().setUpHandler(event.getGuild()); // no point constantly checking for this later
        if(bePlaying && !((AudioHandler) event.getGuild().getAudioManager().getSendingHandler()).isMusicPlaying(
            event.getJDA())) {
        ...
        if(beListening) {
            ...
            if(!userState.inVoiceChannel()
                ...
                event.replyError(
                    "You must be listening in " + (current == null ? "a voice channel" : current.getAsMention())
                        + " to use that!");
                return;
            }
