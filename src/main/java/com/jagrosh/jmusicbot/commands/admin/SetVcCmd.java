...
public class SetVcCmd extends AdminCommand {
    ...
    protected void execute(CommandEvent event) {
        ...
        if(event.getArgs().equalsIgnoreCase("none")) {
            ...
            if(list.isEmpty()) {
                event.reply(
                    event.getClient().getWarning() + " No Voice Channels found matching \"" + event.getArgs() + "\"");
            }
            else if(list.size() > 1) {
                ...
                event.reply(
                    event.getClient().getSuccess() + " Music can now only be played in " + list.get(0).getAsMention());
            }
        }
    }
}
