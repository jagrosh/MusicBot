...
public class SetTcCmd extends AdminCommand {
    ...
    protected void execute(CommandEvent event) {
        ...
        if(event.getArgs().equalsIgnoreCase("none")) {
            ...
            if(list.isEmpty()) {
                event.reply(
                    event.getClient().getWarning() + " No Text Channels found matching \"" + event.getArgs() + "\"");
            }
            else if(list.size() > 1) {
                ...
                event.reply(
                    event.getClient().getSuccess() + " Music commands can now only be used in <#" + list.get(0).getId()
                        + ">");
            }
        }
    }
