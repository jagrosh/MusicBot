    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            builder.setColor(event.getSelfMember().getColor())
                    ...
                    .setSelection((msg, i) -> {
                        ...
                        String addMsg = FormatUtil.filter(
                                event.getClient().getSuccess() + (pos == 0 ? "キューに" + trackTitle + "を追加し、再生を開始します。"
                                        : "キューの" + pos + "番目に" + trackTitle + "を追加しました"));
            ...
            for (int i = 0; i < 4 && i < playlist.getTracks().size(); i++) {
                ...
                builder.addChoices("`[" + FormatUtil.formatTime(track.getDuration()) + "]` [**" + track.getInfo().title
                        + "**](" + track.getInfo().uri + ")");
            }
        ...
        public void noMatches() {
            m.editMessage(
                    FormatUtil.filter(event.getClient().getWarning() + "`" + event.getArgs() + "`に一致する結果は見つかりませんでした。"))
                    .queue();
        }
