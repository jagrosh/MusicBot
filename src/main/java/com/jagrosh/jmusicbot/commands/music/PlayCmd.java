        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist)
        {
            if(bot.getConfig().isTooLong(track))
            {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" This track (**"+track.getInfo().title+"**) is longer than the allowed maximum: `"
                        +FormatUtil.formatTime(track.getDuration())+"` > `"+FormatUtil.formatTime(bot.getConfig().getMaxSeconds()*1000)+"`")).override(true).queue();
                return;
            }
            if(track.getInfo().title.toLowerCase().contains("coldplay") || track.getInfo().author.toLowerCase().contains("coldplay")) {
                m.editMessage("<:jankman_angry:754754504171323515> NO COLDPLAY.").override(true).queue();
                return;
            }
            AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
