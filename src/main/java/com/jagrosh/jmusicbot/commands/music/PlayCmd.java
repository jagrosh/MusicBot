    ...
    public void doCommand(CommandEvent event) {
        if (event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            ...
            builder.append("\n`").append(event.getClient().getPrefix()).append(name)
                    .append(" <曲名>` - YouTubeの最初の検索結果を再生します。");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name)
                    .append(" <URL>` - 指定された曲、プレイリスト、またはストリームを再生します。");
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist) {
            ...
            String addMsg = FormatUtil
                    .filter(event.getClient().getSuccess() + (pos == 0 ? "キューに" + trackTitle + "を追加し、再生を開始します。"
            ...
            if (playlist == null
                ...
                new ButtonMenu.Builder()
                        .setText(addMsg + "\n" + event.getClient().getWarning() + " この曲は曲数**"
                                + playlist.getTracks().size() + "**もあるプレイリストの中にあります。プレイリストを読み込むには" + LOAD
                                + "を選択してください。")
                        ...
                        .setAction(re -> {
                            if (re.getName().equals(LOAD))
                                m.editMessage(addMsg + "\n" + event.getClient().getSuccess() + " 追加曲を**"
                                        + loadPlaylist(playlist, track) + "**件読み込みました。").queue();
                            else
                                m.editMessage(addMsg).queue();
                        }).setFinalAction(m -> {
                            try {
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            if (playlist.getTracks().size() == 1 || playlist.isSearchResult()) {
                ...
                if (playlist.getTracks().size() == 0) {
                    m.editMessage(FormatUtil.filter(event.getClient().getWarning() + "プレイリスト"
                    ...
                    m.editMessage(
                            FormatUtil.filter(event.getClient().getWarning()
        ...
        public void noMatches() {
            if (ytsearch)
                m.editMessage(FormatUtil
                        .filter(event.getClient().getWarning() + "`" + event.getArgs() + "`に一致する結果は見つかりませんでした。"))
                        .queue();
            else
    ...
    public class PlaylistCmd extends MusicCommand {
        ...
        public void doCommand(CommandEvent event) {
            ...
            event.getChannel().sendMessage(
                    loadingEmoji + " プレイリストを読み込み中…**" + event.getArgs() + "** (" + playlist.getItems().size() + " 個)")
                    .queue(m -> {
                        AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
                        playlist.loadTracks(bot.getPlayerManager(),
                                (at) -> handler.addTrack(new QueuedTrack(at, event.getAuthor())), () -> {
                                    StringBuilder builder = new StringBuilder(playlist.getTracks().isEmpty()
                                            ...
                                            : event.getClient().getSuccess() + " 曲を**" + playlist.getTracks().size()
                                                    + "**個読み込みました。");
