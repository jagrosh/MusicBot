...
public class PlayCmd extends MusicCommand {
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist) {
            if (isTrackTooLong(track, this.m, this.event, bot)) return;

            AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
            int pos = Objects.requireNonNull(handler).addTrack(new QueuedTrack(track, event.getAuthor())) + 1;
        ...
        private int loadPlaylist(AudioPlaylist playlist, AudioTrack exclude) {
            ...
            playlist.getTracks().forEach((track) ->
            {
                if (!bot.getConfig().isTooLong(track) && !track.equals(exclude)) {
                    AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
                    Objects.requireNonNull(handler).addTrack(new QueuedTrack(track, event.getAuthor()));
                    count[0]++;
                }
            });
            return count[0];
    ...
    public class PlaylistCmd extends MusicCommand {
        ...
        public void doCommand(CommandEvent event) {
            ...
            {
                AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
                playlist.loadTracks(bot.getPlayerManager(), (at) -> Objects.requireNonNull(handler).addTrack(new QueuedTrack(at, event.getAuthor())), () ->
                {
