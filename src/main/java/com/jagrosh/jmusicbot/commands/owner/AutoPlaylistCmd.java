...
public class AutoPlaylistCmd extends OwnerCommand {
    ...
    public void execute(CommandEvent event) {
        ...
        if(event.getArgs().equalsIgnoreCase("none")) {
            ...
            event.reply(
                event.getClient().getSuccess() + " Cleared the default playlist for **" + event.getGuild().getName()
                    + "**");
            return;
        }
        ...
        if(bot.getPlaylistLoader().getPlaylist(pname) == null) {
            ...
            event.reply(event.getClient().getSuccess() + " The default playlist for **" + event.getGuild().getName()
                + "** is now `" + pname + "`");
        }
    }
}
