...
public class AutoPlaylistCmd extends OwnerCommand {
    ...
    public void execute(CommandEvent event) {
        ...
        if (bot.getPlaylistLoader().getPlaylist(playListName) == null) {
            event.reply(event.getClient().getError() + " Could not find `" + playListName + ".txt`!");
        } else {
            Settings settings = event.getClient().getSettingsFor(event.getGuild());
            settings.setDefaultPlaylist(playListName);
            event.reply(event.getClient().getSuccess() + " The default playlist for **" + event.getGuild().getName() + "** is now `" + playListName + "`");
        }
    }
}
