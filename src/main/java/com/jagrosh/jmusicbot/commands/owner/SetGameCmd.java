...
public class SetGameCmd extends OwnerCommand {
    ...
    protected void execute(CommandEvent event) {
        String title =
            event.getArgs().toLowerCase().startsWith("playing") ? event.getArgs().substring(7).trim() : event.getArgs();
        try {
    ...
    private static class SetlistenCmd extends OwnerCommand {
        ...
        protected void execute(CommandEvent event) {
            ...
            String title =
                event.getArgs().toLowerCase().startsWith("to") ? event.getArgs().substring(2).trim() : event.getArgs();
            try {
